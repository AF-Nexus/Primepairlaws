// server.js - Backend Server with Real Baileys (Fixed)
...
app.post('/api/generate-session', async (req, res) => {
    ...
    try {
        ...
        sock.ev.on('connection.update', (update) => {
            handleConnectionUpdate(update, sessionId, phoneNumber, sessionDir);
        });
        ...
        if (!sock.authState.creds.registered) {
            ...
            try {
                ...
                throw new Error('Failed to request pairing code: ' + codeError.message);
            }
        } else {
            res.status(400).json({
                success: false,
...
// Handle connection updates (separate function to avoid async issues)
async function handleConnectionUpdate(update, sessionId, phoneNumber, sessionDir) {
    ...
    if (connection === 'close') {
        ...
        try {
            ...
            if (fs.existsSync(credsPath)) {
                ...
                await sendSessionToWhatsApp(session.sock, phoneNumber, efSessionId);
...
module.exports = app;    const sessionDir = path.join(__dirname, 'sessions', sessionId);
